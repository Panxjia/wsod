...
class VGG(nn.Module):
    ...
    def get_loss(self, logits, gt_child_label, protype_h=None, protype_v=None, epoch=0, loc_start=10, erase_start=False):
        ...
        if self.args.fpn or self.args.bifpn:
            ...
            sep = True if erase_start and self.args.erase else False
            if sep:
                mask4 = self.erase_mask_54[:,0,...]
                mask3 = self.erase_mask_43[:,0,...]
            else:
                mask4 = mask3 = None
            loss_4 = self.args.loss_w_4 * self.get_loss_fpn(self.cls_map_4, gt_child_label.long(), sep=sep,
                                                            mask=mask4)
            loss += loss_4
            # loss_3 = self.args.loss_w_3 * self.loss_cross_entropy(cls_logits_3, gt_child_label.long())
            loss_3 = self.args.loss_w_3 * self.get_loss_fpn(self.cls_map_3, gt_child_label.long(), sep=sep,
                                                            mask=mask3)
            loss += loss_3
    ...
    def get_loss_fpn(self, logits, gt_child_label,sep=True, mask=None):
        loss = 0
        if sep:
            n, c, h, w = logits.size()
            if self.args.mce:
                ...
                if mask is not None:
                    loss_non_reduce= self.loss_cross_entropy(logits, gt_child_label.long(), reduction='none')
                    loss += torch.mean(loss_non_reduce * mask.detach())
                else:
