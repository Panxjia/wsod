...
class VGG(nn.Module):
    ...
    def get_loc_loss(self, logits, gt_child_label, th_bg=0.3, th_fg=0.5):
        ...
        norm_var_logits = 1 - norm_var_logits
        ...
        # gt_child_label_cp = gt_child_label.clone()
        # gt_child_label_cp = gt_child_label_cp.unsqueeze(-1).unsqueeze(-1)
        # gt_child_label_cp = gt_child_label_cp.repeat(1, lh, lw)
        # gt_child_label_cp[norm_var_logits<th_bg] = 200
        # pos_sam = torch.zeros_like(norm_var_logits)
        # pos_sam[norm_var_logits < th_bg] = 1.
        # pos_sam[norm_fg_cls > th_fg] = 1.

        # cls_mask = torch.ones_like(norm_var_logits)
        # cls_mask[norm_fg_cls < norm_var_logits] = 0.
        cls_mask = -1 * torch.ones_like(norm_var_logits)
        cls_mask[norm_fg_cls > th_fg] = 1.
        cls_mask[norm_var_logits > (1 - th_bg) ] = 0.
