...
def get_arguments():
    ...
    parser.add_argument("--g2cam", action='store_true', help='return g2cam.')
...
def get_grad(model, logits, feat, num_maps, layer='cls3'):
    grads = {}
    pred_cls = logits.argmax(dim=1, keepdim=True)
    grad_traget_map = torch.zeros_like(logits)
    grad_traget_map[pred_cls] = 1
    model.zero_grad()
    logits.backward(grad_traget_map, retain_graph=True)
    grads['gg_{}'.format(layer)] = model.module.guided_grad.cpu().data.numpy()
    last_layer_grad = model.module.last_layer_grad_out[layer]
    n, _, h, w = last_layer_grad.size()
    last_layer_grad_w = last_layer_grad.view(n, -1, num_maps, h, w).mean(dim=3,keepdim=True).mean(dim=4, keepdim=True)
    feat = feat.view(n, -1, num_maps, h, w)
    gcam = (feat*last_layer_grad_w).sum(dim=2)/last_layer_grad_w.sum(dim=2)
    if layer != 'cls5':
        gcam = F.interpolate(gcam, size=(28, 28), mode='bilinear', align_corners=True)
    grads['gcam_{}'.format(layer)] = gcam.cpu().data.numpy()
...
def val(args):
    ...
    for dat in tqdm(valcls_loader):
        ...
        if args.tencrop == 'True':
            ...
            label = label_in
        grads[os.path.basename(img_path[0])] = {}
        # forward pass
        img, label = img.cuda(), label.cuda()
        # img_var, label_var = Variable(img), Variable(label)
        logits = model(img.requires_grad_())
    ...
    for th in thresholds:
        ...
        for idx, dat in tqdm(enumerate(valloc_loader)):
            ...
            # img_var, label_var = Variable(img), Variable(label)
            logits = model(img)
            child_map = F.interpolate(model.module.get_child_maps(), size=(28, 28), mode='bilinear', align_corners=True)
            child_map = child_map.cpu().data.numpy()
            parent_maps = F.interpolate(model.module.get_parent_maps(), size=(28, 28), mode='bilinear', align_corners=True)
